...
// Read-only by-pointer accessor: lets a test observe that a plain Python object
// lazily crossed into C as `struct hello_world *` carried the right field values.
double sum_hw(struct hello_world *hw)
{
    return hw->hello + hw->world;
}
