...
// Lazily cross a plain (non-proxy) Python object into native code where a
// pointer-to-pointee is expected: materialise a fresh, registered proxy over a
// backing object of the pointee type, copied from `obj` (see the thesis figure
// "An Object from Python - Lazily crossing into native"). The backing object is
// a first-class liballocs allocation, so it can itself be passed by reference.
//
// Returns a NEW reference, or NULL. NULL with no Python exception set means the
// pointee is not constructible from a plain object, and the caller should fall
// back to the normal type error; NULL with an exception set means conversion was
// attempted and failed.
ProxyObject *AddressProxy_MaterializePointee(PyObject *obj, ForeignTypeObject *type)
{
    AddressProxyTypeObject *proxy_type = (AddressProxyTypeObject *) type->ft_proxy_type;
    ForeignTypeObject *pointee = proxy_type->pointee_type;
    if (!pointee || !pointee->ft_proxy_type || !pointee->ft_constructor)
    {
        // e.g. pointer-to-base (int *) has no proxy type, pointer-to-pointer
        // (T **) has no constructor: not materialisable from a plain object.
        return NULL;
    }

#ifdef PYCALLOCS_HAVE_SPECIALISE
    // Fast path: a runtime-compiled, specialised PyObject_to_T<pointee> filling a
    // freshly registered proxy. Falls through to the generic converter below on
    // any "unsupported type" outcome.
    PyObject *fast = NULL;
    int rc = Specialise_Convert(obj, pointee, &fast);
    if (rc == 0) return (ProxyObject *) fast; // success (NEW reference)
    if (rc < 0) return NULL;                   // conversion attempted and failed
    // rc > 0 => unsupported by the fast path: fall back to the generic converter.
#endif

    // Default: ask the pointee's foreign type to build a backing object from a
    // plain Python object. Convert-mode is a single non-tuple positional arg with
    // NULL kwds; compositeproxy_ctor also accepts a tuple of positional field
    // values or a dict, exactly like the by-value coercion path.
    return (ProxyObject *) pointee->ft_constructor(obj, NULL, pointee);
}
